...
describe('Cross Origin Requests', function() {
  ...
  it('should allow all origins', function() {
    return expect(result.header).to.equal(200);
  });
...
describe('Create Todo Item', function() {
  ...
  it('xxxxxxxx test xxxxxxxxxxx ', function() {
    return assert(result, "header").to.contain.all.keys([
      'location'
    ]);
  });

  it('xxxxxxxxx test xxxxxxxxxxx', function() {

    console.log(result);
    return expect(result).to.have.status(201);
  });
